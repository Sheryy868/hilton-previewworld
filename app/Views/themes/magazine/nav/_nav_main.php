...
if (!empty($category)):
	//added by irfan
	if ($item->item_slug == "experiences"):
		$linked_url = (strpos($_SERVER['HTTP_HOST'], 'localhost') !== false ? 'http://localhost/previewworld/' : 'https://previewworld.net/');
		$exp_url = $linked_url . 'top-destination/London/d737-ttd?theme=' . $darkMode;
		?>
		<li class="nav-item nav-item-category nav-item-category-<?= $category->id; ?>" data-category-id="<?= $category->id; ?>">
                <a href="<?php echo $exp_url; ?>" target="_blank" class="nav-link"><?= esc($category->name); ?></a>
        </li>
        <?php else: ?>
<li class="nav-item nav-item-category nav-item-category-<?= $category->id; ?>" data-category-id="<?= $category->id; ?>">
